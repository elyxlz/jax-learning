...
def rms_norm_bwd(eps, res, g):
    invvar, x, weight = res
    grad_input, grad_weight, part_grad = _rms_norm_bwd_p.bind(g, invvar, x, weight, eps=eps)
...
def _rms_norm_bwd_abstract(grad_output, invvar, x, weight, eps):
    ...
    assert iv_dtype == jnp.float32 if x_dtype in [jnp.float16, jnp.bfloat16] else x_dtype
    assert grad_output.named_shape == x.named_shape
    weight_named_shape = weight.named_shape if weight.named_shape else grad_output.named_shape
    return (
        ShapedArray(x.shape, x_dtype, named_shape=x.named_shape),  # grad input
        ShapedArray(weight.shape, w_dtype, named_shape=weight_named_shape),  # grad weight
        ShapedArray(part_grad_shape, iv_dtype, named_shape=weight_named_shape),  # part grad
    )
...
def _check_valid_batch_dims(bdims):
    ...
    for dim in bdims:
        assert dim in [0, None], (
            "Currently only support batch_dim in [0, None], " f"but got {dim=}"
        )
...
class RmsNormBwdClass:
    ...
    def impl(grad_output, invvar, x, weight, eps):
        ...
        gx, gw, part_grad = RmsNormBwdClass.inner_primitive.bind(
            grad_output, invvar, x, weight, eps=eps
        )
...
), "Only 1 GPU, the example work, but it is this really what you want?"
x = jax.random.normal(
